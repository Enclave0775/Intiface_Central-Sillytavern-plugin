...
*   **視覺高亮：**
    ...
    *   *顏色可在設定中自定義。*
*   **自訂播放與循環模式 (Playlist & Loop Modes)：**
    *   **A-B 片段播放：** 可以在訊息旁設定「起點 (Flag)」與「終點 (Checkered Flag)」，只播放指定的對話區間。
    *   **播放清單循環 (預設)：** 播放到底後（或到達設定的終點後），自動回到第一句（或設定的起點）重新循環。
    *   **只循環最後一條：** 僅不斷重複播放最後一則訊息。
...
3.  **連接插件：**
    ...
    *   插件會自動掃描並列出已連接的設備。
4.  **設定：**
    *   **循環執行指令 (Loop Patterns)：** 勾選此項可在訊息播放完畢後重複執行。取消勾選則停止循環。（注意：手動暫停裝置也會自動取消勾選此選項）。
        *   **播放清單循環 (Full Playlist)：** 從觸發的訊息開始播放到底，然後重新開始。
        *   **只循環最後一條 (Last Message Only)：** 僅重複最後一則訊息。
